...
def main(cfg: argparse.Namespace):
    ...
    if cfg.resume:
        try:
            ...
            # "epoch_145.keras" → 145 → initial_epoch=145 (start next epoch)
            epoch_str = Path(cfg.resume).stem.split("_")[-1]
            initial_epoch = int(epoch_str)
            print(f"[INFO] Resumed from {cfg.resume} (initial_epoch={initial_epoch})")
        except Exception as e:
            print(f"[WARN] Failed to load resume checkpoint {cfg.resume}: {e}")
